...
exports.updateWeekSums = function(rootRef, target) {
  ...
  const absences_pr = rootRef.child(getPathToAbsences(accountID, weekID)).orderByChild('user').equalTo(userID).once('value')

  return Promise.all([shifts_pr, absences_pr]).then(results => {

    const miniShifts = results[0].val()
    const absences   = results[1].val() // its absences becauste a user could have more than 1 absence in week
